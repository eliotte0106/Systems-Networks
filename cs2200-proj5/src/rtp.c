...
packet_t *packetize(char *buffer, int length, int *count) {
    ...
    if (!(length % MAX_PAYLOAD_LENGTH)){
        p_count = length / MAX_PAYLOAD_LENGTH;
    } else{
        p_count = length / MAX_PAYLOAD_LENGTH + 1;
    }
    *count = p_count;
    packets = calloc((unsigned long) p_count, sizeof(packet_t));
