...
char *generateMessage()
{
    ...
    for (int i = 0; i < length - 1; i++)
    {
        ...
        add_at_index(message, word, i); // fixed#1
    }
    ...
    for(int i = 0; i < length; i++) {
        ...
        //total_size = word_size + 1 + ((string_message == NULL)? 0 : strlen(string_message)); // incorrect
        //total_size = strlen(string_message) + 1 + ((i == 0)? word_size : word_size + 1); //origin code
        total_size = word_size + 2 + ((i == 0)? 0 : strlen(string_message)); // fixed #2 correct
