...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        pte_t *pte = (pte_t *)(mem + (frame_table[victim_pfn].process->saved_ptbr * PAGE_SIZE) + (frame_table[victim_pfn].vpn * sizeof(pte_t)));
        if (pte->dirty) {
            stats.writebacks++;
...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        for (int i = 1; i <= num_entries * 2; i++) {
            pfn_t victim = (last_evicted + i) % num_entries;
            if (!frame_table[victim].protected && !frame_table[victim].referenced) {
                last_evicted = victim;
                return victim;
            } else if (frame_table[victim].referenced) {
                frame_table[victim].referenced = 0;
            }
        }
    }
    /*/
    By iterating num_entries * 2 times, the algorithm ensures that the clock hand will sweep through every frame in the table twice
    * which ensures that all referenced frames have their reference bit cleared, and all frames have been checked for eviction
    **/
